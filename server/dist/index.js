...
app.get("/", (_req, res) => {
    res.type("text/html; charset=utf-8").send(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Women Registration Program API</title></head><body>
...
app.post("/upload", requireBearer, upload.single("file"), async (req, res) => {
    try {
        ...
        return res.status(503).json({
            success: false,
            message: "S3 not configured: set AWS_S3_BUCKET, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY (and optional AWS_S3_PUBLIC_BASE_URL for CloudFront).",
        });
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
